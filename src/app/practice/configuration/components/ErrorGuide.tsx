...
interface ErrorGuideItem {
  ...
  description: string;
  solution: string;
  category: "router" | "switch" | "pc" | "general";
  commands?: string[];
}

export default function ErrorGuide() {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<ErrorGuideItem["category"] | "all">("all");
  ...
  const filteredGuides = errorGuides.filter(guide => {
    const matchesSearch = guide.error.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="space-y-4">
            <div className="flex gap-4">
              ...
              <select
                ...
                onChange={(e) => setSelectedCategory(e.target.value as ErrorGuideItem["category"] | "all")}
                ...
                <div key={index} className="border border-gray-700 rounded-lg p-4">
                  ...
                  {guide.commands && (
                    <div className="mt-4 bg-black p-4 rounded font-mono text-sm">
                      <h4 className="text-sm font-semibold mb-2 font-sans">Useful Commands:</h4>
                      {guide.commands.map((cmd, i) => (
                        <div key={i} className="text-green-400 mb-1">$ {cmd}</div>
                      ))}
                    </div>
                  )}
                </div>
