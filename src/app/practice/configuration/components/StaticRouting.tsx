...
const VALID_COMMANDS: Command[] = [
  ...
  },
  {
    ...
    requiredMode: "config",
    nextCommands: ["end"],
    help: "Configure static route",
    suggestion: "Use 'ip route <network> <mask> <next-hop>'",
    instructions: [
      "1. Configure static route with network, mask, and next-hop",
    ],
  },
  {
    command: "end",
...
export default function StaticRouting() {
  ...
  const [routesAddedViaUI, setRoutesAddedViaUI] = useState(false);
  ...
  useEffect(() => {
    ...
    if (routesAddedViaUI) {
      // If routes were added via UI, only allow show ip route and ping
      currentSuggestions.push(
        ...
        "Option 2:",
        "→ ping <ip-address>",
        "(Ping a destination IP)",
        ""
      );
    } else {
      // If routes were not added via UI, allow full CLI process
      currentSuggestions.push(
        ...nextCommands
          .map((cmd, index) => [
            `Option ${index + 1}:`,
  ...
  }, [mode, commandProgress, error, routesAddedViaUI]);
  ...
  const validateCommand = (
    ...
    // If routes were added via UI, block configure terminal and ip route
    if (
      routesAddedViaUI &&
      ["configure terminal", "ip route"].includes(validCommand.command)
    ) {
      return {
        valid: false,
        error: "Command not available after routes are added via UI",
        help: "You can only use 'show ip route' and 'ping' after adding routes via UI",
      };
    }
  ...
  const handleAddRoute = () => {
    ...
    // Mark routes as added via UI
    setRoutesAddedViaUI(true);
  };
